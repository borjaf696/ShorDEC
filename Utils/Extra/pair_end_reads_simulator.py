...
def _add_fails_read(read):
	global fail_on_reads, nuc
	l_read = list(read[0:len(read)-1])
	l_read = [l_read[i] if t==0 else l_read[i] for i,t in enumerate(np.random.binomial(1,fail_on_reads,len(l_read)))]
	read = ("").join(l_read)
	return read
	

def _cut_read(read):
	global read_size, insert_size, max_delta, len_var, nuc
	left_reads, right_reads = [], []
	left_read, right_read = "",""
	for i in range(0,len(read)-insert_size-read_size, read_size):
		print i
		...
		left_read, right_read = "",""
	for j,read in enumerate(left_reads):
		read = [read[i] if t==0 else nuc[randint(0,3)] for i,t in enumerate(np.random.binomial(1,fail_on_reads,len(read)))]
		left_reads[j] = ("").join(read)
	for j,read in enumerate(right_reads):
		read = [read[i] if t==0 else nuc[randint(0,3)] for i,t in enumerate(np.random.binomial(1,fail_on_reads,len(read)))]
		left_reads[j] = ("").join(read)
...
def add_errors(reads):
	...
	for i,read in enumerate(final_reads):
		final_reads[i] = _add_fails_read(read)
