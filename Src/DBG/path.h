...
#define MAX_ERROR_RATE 0.3
#define DELTA_EXTENSION 0.1
#define ERROR_HEADTAIL_RATE 0.2
 ...
 * TODO: Change name because path is not exactly what it is.
 * */
...
public:
    ...
    Path()
    {
        ...
        for (uint i = 0; i < (MAX_PATH_LEN+1); ++i) {
            _DP[i] = i;
            _DP[i*(MAX_PATH_LEN+1)] = i;
        }
    }
    ~Path()
    {
...
public:
    ...
    int check_solids(size_t,size_t,size_t,size_t
            ,size_t &,Kmer&);
private:
    vector<KmerInfo<P>> _solid;
    unordered_set<KmerInfo<P>> _origin;
    bool _head = true, _tail = true;
