...
DnaSequence PathContainer<false>::correct_read() {
    ...
    if (!no_solids)
    {
        ...
        DnaSequence full_path(seq_head.str() + path_graph.shortest_path(first_kmer, last_kmer).str() + seq_tail.str());
        ...
        for (auto &read: _sc.getIndex())
        {
            #pragma omp task shared(read)
            {
                ...
                if (read.first.getId() % 2 == 0)
                {
                    ...
                    _sc.setRead(read.first.getId(),pc.correct_read());
                }
            };
        }
    };
